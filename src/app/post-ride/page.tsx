              ...
              <div key={ride.id} style={{
                ...
                <div style={{ borderTop: `1px solid ${T.border}`, padding: '5px 16px', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6 }}>
                    {isFull ? (
                      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.85)', fontWeight: 700 }}>پڕە</span>
                    ) : (
                      <span style={{
                ...
                {isOpen && (
                  <div style={{ padding: '10px 16px' }}>
                    <div style={{ padding: '8px 12px', background: T.cardInner, borderRadius: 10, fontSize: 11, color: T.textMid, lineHeight: 2, marginBottom: ride.notes ? 8 : 0 }}>
                      ...
                      <div>جێگای بەردەست: <span style={{ color: ride.available_seats > 0 ? '#ccc' : 'rgba(255,255,255,0.85)', fontSize: ride.available_seats > 0 ? undefined : 12, fontWeight: ride.available_seats > 0 ? undefined : 700 }}>{ride.available_seats > 0 ? `${ride.available_seats} جێ` : 'پڕە'}</span></div>
                    </div>
                    {ride.notes && (
